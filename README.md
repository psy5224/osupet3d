...
3D 변환은 Meshy 6의 고정밀 원본 메시 설정(`should_remesh: false`)과 이미지 보정 비활성화(`image_enhancement: false`)를 사용합니다. 2D 생성 단계에서도 실제 머리·몸통·다리 비율과 각 신체 부위의 경계를 선명하게 유지합니다. 다만 한 장의 이미지에 보이지 않는 옆면과 뒷면은 AI가 추정하므로 2D와 완전히 동일한 체형을 수학적으로 보장할 수는 없습니다.
