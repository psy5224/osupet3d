...
STANCE - MANDATORY
Show the pet calmly standing completely still in a neutral, natural species-appropriate 45-degree stance, regardless of any pose in the input photos or style reference. The entire body must be visible from ears to tail and paws. All four legs and all four paws must be clearly visible as four distinct, anatomically correct limbs. Stagger the near and far legs naturally in the three-quarter view, leaving visible white separation so no front leg, hind leg, or paw is hidden behind another limb or the torso. All four paws must be firmly and evenly planted on the same invisible horizontal ground plane. Distribute the pet's body weight equally across all four legs and all four paws: every leg must be straight but naturally relaxed, vertical, stable, and visibly load-bearing, with the full surface of every paw contacting the ground evenly. Keep the shoulders, spine, chest, abdomen, and hips level and centered. Do not shift weight toward the front, back, left, or right; do not lean, tilt the pelvis, drop one hip or shoulder, bend one supporting leg, stand on tiptoe, or make any paw look light, hovering, or ready to step. No paw may be lifted. No walking, trotting, running, jumping, leaping, floating, sitting, lying down, crouching, play bow, rearing, dancing, dynamic action pose, head tilt, motion lines, or wind-swept motion.

TAIL SEPARATION - MANDATORY
The tail must connect to the body only at its anatomically correct root on the rear pelvis. Immediately after that short root connection, it must extend outward toward the viewer's right and then curve upward, entirely outside the torso, rump, and hind-leg silhouette. After the root connection, maintain a clearly visible pure-white gap approximately one paw-width or wider between the tail and the torso, rump, and hind legs. Never let the tail cross in front of or behind the torso silhouette, rest against or curl over the back, merge into the hip, touch a hind leg, disappear behind fur, or look embedded in the body. Keep the complete tail visible and uncropped. Preserve the photographed pet's true tail length, thickness, fur texture, and natural species-appropriate curve, including a naturally short tail, while prioritizing unmistakable separation for single-image 3D reconstruction. Never let the tail hang down, tuck between the legs, or lie on the floor.
